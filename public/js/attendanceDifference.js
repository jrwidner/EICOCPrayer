    ...
    fetch('/api/attendance-difference')
        ...
        .then(data => {
            ...
            data.forEach(record => {
                ...
                uniqueDates.add(new Date(`${record.Date}T00:00:00Z`).toLocaleDateString());
            ...
            const renderTable = (selectedMembers, worshipFilterCriteria = 'all') => {
                ...
                sortedDates.forEach(date => {
                    const totalWorshipAttendees = data.filter(record => new Date(`${record.Date}T00:00:00Z`).toLocaleDateString() === date && record.WorshipService).length;
                    const totalBibleClassAttendees = data.filter(record => new Date(`${record.Date}T00:00:00Z`).toLocaleDateString() === date && record.BibleClass).length;
                ...
                filteredNames.forEach((name, index) => {
                    ...
                    if (selectedMembers.length === 0 || selectedMembers.includes(name)) {
                        ...
                        sortedDates.forEach(date => {
                            const record = records.find(record => new Date(`${record.Date}T00:00:00Z`).toLocaleDateString() === date);
                            if (record) {
                    ...
                    datasets: [
                        {
                            ...
                            data: sortedDates.map(date => data.filter(record => new Date(`${record.Date}T00:00:00Z`).toLocaleDateString() === date && record.WorshipService).length),
                        ...
                        },
                        {
                            ...
                            data: sortedDates.map(date => data.filter(record => new Date(`${record.Date}T00:00:00Z`).toLocaleDateString() === date && record.BibleClass).length),
